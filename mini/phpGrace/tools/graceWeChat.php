...
define('GWECHAT_VALIDTOKEN',  '******');        //Token 用于接口认证
...
//小程序支付配置
define('GWECHAT_WXPAY_XCX_APPID',  'wx41245dd430035de4'); //小程序 appId 与 微信支付对应
define('GWECHAT_WXPAY_XCX_MCHID',  '1547040331');       //微信支付对应的商户ID
define('GWECHAT_WXPAY_XCX_KEY',    'wghfjhvfuytfrkrewstfytjvhjvugbjh'); //微信支付对应的KEY

//小程序获取用户信息的必须配置
define('GWECHAT_XCX_APPID',  'wx41245dd430035de4'); //小程序 appId 与 微信支付对应
define('GWECHAT_XCX_SECRET', '07603983645cde794fd57c44db9d2975'); //小程序对应的 SECRET
...
class graceWeChat{
	...
	public function getOrder($transaction_id, $type = 'JSAPI'){
		...
		$array['sign']                   = $this->sign($array, $type);
...
<MsgType><![CDATA[text]]></MsgType><Content><![CDATA['.$msg.']]></Content></xml>';
    ...
    public function createJsBizPackage($openid, $totalFee, $outTradeNo,$trueName=''){
        $unified = array(
            'mch_appid' => GWECHAT_WXPAY_XCX_APPID,
            'mchid' => GWECHAT_WXPAY_XCX_MCHID,
		...
		$unified['sign'] = $this->sign($unified, 'XCX');
		$xml = $this->arrayToXml($unified);
		// return $xml;
		$url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers';
		$res = $this->curlPost($url, $xml, true);
		$arr = $this->xmlToArray($res);
		return $arr;
		if($arr['return_code'] == 'FAIL'){exit(json_encode(array('status' =>'error', 'msg'=>$arr['return_msg'])));}
		if($arr['result_code'] == 'FAIL'){exit(json_encode(array('status' =>'error', 'msg'=>$arr['err_code_des'])));}
	 ...
	 * 查询企业付款
	 */
	public function getJsBizPackage($outTradeNo){
		$unified = array(
            'appid' => GWECHAT_WXPAY_XCX_APPID,
            'mch_id' => GWECHAT_WXPAY_XCX_MCHID,
            'nonce_str' => $this->createNonceStr(),
            'partner_trade_no' => $outTradeNo
        );
		$unified['sign'] = $this->sign($unified, 'XCX');
	 ...
	 * 参数2 $data 格式 array('name'=>'test', 'age' => 18)
	 */
	public function curlPost($url, $data, $useCert = false){
	    $ch = curl_init();
	    curl_setopt($ch, CURLOPT_URL, $url);
	    curl_setopt($ch, CURLOPT_RETURNTRANSFER , true);
	    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER , false);
		...
		if($useCert == true){
        	...
        	curl_setopt($ch,CURLOPT_SSLCERT,dirname(__FILE__).PG_DS.'../wxpaycert/apiclient_cert.pem');
        	//默认格式为PEM，可以注释
        	curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
        	curl_setopt($ch,CURLOPT_SSLKEY,dirname(__FILE__).PG_DS.'../wxpaycert/apiclient_key.pem');
		}
	    $res  = curl_exec($ch);
	    curl_close($ch);
	    return $res;
	}
	public function certcurlPost($url = '', $postData = '', $options = array())
    {
